...
// ═══════════════════════════════════════════════
import { addDays, callFn, COMMON_RULES, dow, LLMImage, num, reportSchema, rest, Row, safeCollector, serveAgent } from "../_shared/agent.ts";
...
async function collect(D: string) {
  ...
  const cards = adIds.size ? await safe("광고 소재 카드", () => callFn("meta-ads", { action: "adcards", ad_ids: [...adIds].slice(0, 60).join(","), start_date: cur14[0], end_date: cur14[1] })) : null;
...
function forLLM(data: Row) {
  ...
  return { ...data, new_arrivals: { ...na, matrix }, focus };
}
...
3) actions는 오늘 실행할 구체적인 것 3개(광고팀·상품팀 위주).
${COMMON_RULES}`;
...
serveAgent({ agent: AGENT, label: "상품 전략 담당", system: SYSTEM, schema: SCHEMA, collect, forLLM, postProcess, images });
